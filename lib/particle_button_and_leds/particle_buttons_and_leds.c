...
#include <nrfx.h>
#include <particle_buttons_and_leds.h>

LOG_MODULE_REGISTER(particle_buttons_and_leds, CONFIG_PARTICLE_LIBRARY_LOG_LEVEL);
...
static int callback_ctrl(bool enable)
{
	...
	for (size_t i = 0; (i < ARRAY_SIZE(button_pins)) && !err; i++) {
		if (enable) {
			flags = (IS_ENABLED(CONFIG_PARTICLE_LIBRARY_INVERT_BUTTONS) ?
...
static uint32_t get_buttons(void)
{
	...
	for (size_t i = 0; i < ARRAY_SIZE(button_pins); i++) {
		...
		if ((val && !IS_ENABLED(CONFIG_PARTICLE_LIBRARY_INVERT_BUTTONS)) ||
		    (!val && IS_ENABLED(CONFIG_PARTICLE_LIBRARY_INVERT_BUTTONS))) {
			ret |= 1U << i;
		}
	}
...
static void button_handlers_call(uint32_t button_state, uint32_t has_changed)
{
	...
	if (IS_ENABLED(CONFIG_PARTICLE_LIBRARY_DYNAMIC_BUTTON_HANDLERS)) {
...
static void buttons_scan_fn(struct k_work *work)
{
	...
	if (button_scan != 0) {
		k_work_reschedule(&buttons_scan,
		  K_MSEC(CONFIG_PARTICLE_LIBRARY_BUTTON_SCAN_INTERVAL));
...
int dk_buttons_init(button_handler_t button_handler)
{
	...
	gpio_flags_t flags = (IS_ENABLED(CONFIG_PARTICLE_LIBRARY_INVERT_BUTTONS) ?
...
#ifdef CONFIG_PARTICLE_LIBRARY_DYNAMIC_BUTTON_HANDLERS
...
int dk_set_leds_state(uint32_t leds_on_mask, uint32_t leds_off_mask)
{
	...
	for (size_t i = 0; i < ARRAY_SIZE(led_pins); i++) {
		if ((BIT(i) & leds_on_mask) || (BIT(i) & leds_off_mask)) {
			...
			if (IS_ENABLED(CONFIG_PARTICLE_LIBRARY_INVERT_LEDS)) {
				val = 1 - val;
			}
...
int dk_set_led(uint8_t led_idx, uint32_t val)
{
	...
	err = gpio_pin_set_raw(led_devs[led_idx], led_pins[led_idx].number,
		IS_ENABLED(CONFIG_PARTICLE_LIBRARY_INVERT_LEDS) ? !val : val);
	if (err) {
		LOG_ERR("Cannot write LED gpio");
	}
	return err;
